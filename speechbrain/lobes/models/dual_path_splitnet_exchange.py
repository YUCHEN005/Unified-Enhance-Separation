...
class Split_Block(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_spks):
            self.dual_path_blocks.append(
                copy.deepcopy(dual_path_block)
            )
        ...
        # Exchange module
        self.exchane_module = ExchangeModule(out_channels)
    ...
    def forward(self, x):
        ...
        y : list [B, N, K, S], [B, N, K, S], ...
            ...
            where, B = Batchsize,
               ...
               S = the number of chunks
        """
        assert len(x) == self.num_spks

        B, N, K, S = x[0].shape
        assert x[1].shape == (B, N, K, S)

        out = []
        for i in range(self.num_spks):
            out.append(self.dual_path_blocks[i](x[i]))

        return self.exchane_module(out)
...
class ExchangeModule(nn.Module):
    def __init__(
        ...
        self.norm1 = select_norm(norm, out_channels, 4)
        self.norm2 = select_norm(norm, out_channels, 4)

        self.conv_q1 = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, (1, 1), (1, 1)),
    ...
    def forward(self, x):
        ...
        q1 = self.conv_q1(x1).permute(0, 3, 2, 1).contiguous().view(B * S, K, N)
        k1 = self.conv_k1(x1).permute(0, 3, 1, 2).contiguous().view(B * S, N, K)
        v1 = self.conv_v1(x1).permute(0, 3, 2, 1).contiguous().view(B * S, K, N)

        q2 = self.conv_q2(x2).permute(0, 3, 2, 1).contiguous().view(B * S, K, N)
        k2 = self.conv_k2(x2).permute(0, 3, 1, 2).contiguous().view(B * S, N, K)
        v2 = self.conv_v2(x2).permute(0, 3, 2, 1).contiguous().view(B * S, K, N)

        q1k2 = torch.softmax(torch.matmul(q1, k2) / math.sqrt(N), dim=-1)               # (B*S, K, K)
        r12 = torch.matmul(q1k2, v2).view(B, S, K, N).permute(0, 3, 2, 1).contiguous()  # (B, N, K, S)
        r12 = self.conv1(r12)

        q2k1 = torch.softmax(torch.matmul(q2, k1) / math.sqrt(N), dim=-1)               # (B*S, K, K)
        r21 = torch.matmul(q2k1, v1).view(B, S, K, N).permute(0, 3, 2, 1).contiguous()  # (B, N, K, S)
        r21 = self.conv2(r21)
...
class Dual_Path_Model(nn.Module):
    ...
    def __init__(
        ...
        self.split_mdl = nn.ModuleList([])
        for i in range(num_layers):
            self.split_mdl.append(
                copy.deepcopy(
                    Split_Block(
                        num_spks,
                        dual_path_block,
    ...
    def forward(self, x):
        ...
        # [B, N, K, S], [B, N, K, S], ...
        input = [x] * self.num_spks

        # [B, N, K, S], [B, N, K, S], ...
        for i in range(self.num_layers):
            input = self.split_mdl[i](input)

        for i in range(self.num_spks):
            input[i] = self.prelu(input[i])
            input[i] = self.conv2d(input[i])
            # [B, N, K, S] -> [B, N, L]
            input[i] = self._over_add(input[i], gap)
            input[i] = self.output(input[i]) * self.output_gate(input[i])
            input[i] = self.end_conv1x1(input[i])
            input[i] = self.activation(input[i])
