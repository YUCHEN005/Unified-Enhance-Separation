...
class CrossTransformerEncoderLayer(nn.Module):
    ...
    -------
    >>> import torch
    >>> x1 = torch.rand((8, 60, 512))
    >>> x2 = torch.rand((8, 30, 512))
    >>> net = CrossTransformerEncoderLayer(512, 8, d_model=512)
    >>> output = net(x1, x2)
    >>> output[0].shape
    torch.Size([8, 60, 512])
    """
    ...
    def __init__(
        ...
        self.norm0 = sb.nnet.normalization.LayerNorm(d_model, eps=1e-6)
        self.norm1 = sb.nnet.normalization.LayerNorm(d_model, eps=1e-6)
        self.norm2 = sb.nnet.normalization.LayerNorm(d_model, eps=1e-6)
    ...
    def forward(
        ...
        if self.normalize_before:
            ...
            src1 = src
        output, cross_attn = self.cross_att(
            src1,
            memory,
            memory,
            attn_mask=src_mask,
...
class CrossTransformerEncoder(nn.Module):
    ...
    >>> x1 = torch.rand((8, 60, 512))
    >>> x2 = torch.rand((8, 30, 512))
    >>> net = CrossTransformerEncoder(1, 8, 512, d_model=512)
    >>> output, _ = net(x1, x2)
    >>> output.shape
    torch.Size([8, 60, 512])
    """
    ...
    def forward(
        ...
        """
        output = src
        self_attention_lst, cross_attention_lst = [], []
        for enc_layer in self.layers:
            output, self_attention, cross_attention = enc_layer(
                output,
                memory,
                src_mask=src_mask,
                src_key_padding_mask=src_key_padding_mask,
            ...
            self_attention_lst.append(self_attention)
            cross_attention_lst.append(cross_attention)
        output = self.norm(output)

        return output, self_attention_lst, cross_attention_lst
...
class CrossTransformerEncoderLayer2(nn.Module):
    ...
    >>> x1 = torch.rand((8, 60, 512))
    >>> x2 = torch.rand((8, 30, 512))
    >>> net = CrossTransformerEncoderLayer2(512, 8, d_model=512)
    >>> output = net(x1, x2)
    >>> output[0].shape
    torch.Size([8, 60, 512])
    """
    ...
    def forward(
        ...
        return src, self_attn, cross_attn

class CrossTransformerEncoder2(nn.Module):
    """This class implements the transformer encoder.
    ...
    >>> x1 = torch.rand((8, 60, 512))
    >>> x2 = torch.rand((8, 30, 512))
    >>> net = CrossTransformerEncoder2(1, 8, 512, d_model=512)
    >>> output, _ = net(x1, x2)
    >>> output.shape
    torch.Size([8, 60, 512])
    """
...
class GuidedTransformerEncoderLayer(nn.Module):
    ...
    >>> x1 = torch.rand((8, 60, 512))
    >>> x2 = torch.rand((8, 30, 512))
    >>> net = GuidedTransformerEncoderLayer(512, 8, d_model=512)
    >>> output = net(x1, x2)
    >>> output[0].shape
    torch.Size([8, 60, 512])
    """
...
class GuidedTransformerEncoder(nn.Module):
    ...
    >>> x1 = torch.rand((8, 60, 512))
    >>> x2 = torch.rand((8, 30, 512))
    >>> net = GuidedTransformerEncoder(1, 8, 512, d_model=512)
    >>> output, _ = net(x1, x2)
    >>> output.shape
    torch.Size([8, 60, 512])
    """
