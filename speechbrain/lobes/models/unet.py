...
class PEGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        cnn_feat = x   # [B, N, L]
        if self.s == 1:
            x = self.proj(cnn_feat) + cnn_feat   # [B, N, L]
        else:
            x = self.proj(cnn_feat)   # [B, N, L//s]
...
class SepformerBlock(nn.Module):
    """The UNet model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> sepformerBlock = SepformerBlock(64, 64, intra_block, inter_block, num_spks=2)
    ...
    def __init__(
        ...
        # PEG
        self.peg = PEGenerator(out_channels, out_channels)
...
class DownSampleBlock(nn.Module):
    def __init__(
        ...
        self.proj = nn.Linear(in_channels*2, in_channels*2, bias=False)    
...
class UpSampleBlock(nn.Module):
    def __init__(
        ...
        self.proj = nn.Linear(in_channels, in_channels//2, bias=False)
        
    
    def forward(self, x, downsample):
        # x: [B, N, L]
        # downsample: [B, N//2, 2*L]
        B, N, L = x.shape
        assert downsample.shape == (B, N//2, 2*L)

        # [B, N//2, 2*L]
        x = rearrange(x, 'b (p n) l-> b n (l p)', p=2)
...
class UNet_Model(nn.Module):
    ...
    num_layers : int
        Number of layers of Dual Computation Block.
    num_blocks: int
        Number of up/down sampling blocks of UNet.
    norm : str
        Normalization type.
    K : int
        Chunk length.
    ...
    Example
    ---------
    >>> unet = UNet_Model(in_channels=128, out_channels=128, d_model=128, num_spks=2)
    >>> x = torch.randn(10, 128, 2000)
    >>> x = unet(x)
    >>> x.shape
    torch.Size([2, 10, 128, 2000])
    """
    ...
    def __init__(
        ...
        for i in range(self.num_blocks):
            self.downsample.append(DownSampleBlock(in_channels * (2 ** i)))
            self.upsample.append(UpSampleBlock(in_channels * (2 ** (self.num_blocks - i))))
        ...
        intra_model = SBTransformerBlock(
            num_layers=num_transformer_layers,
            d_model=d_model * (2 ** self.num_blocks),
            nhead=nhead,
            d_ffn=d_ffn,
            dropout=dropout,
            use_positional_encoding=use_positional_encoding,
        ...
        self.bottleneck = copy.deepcopy(
            SepformerBlock(
                in_channels * (2 ** self.num_blocks),
                out_channels * (2 ** self.num_blocks),
                intra_model,
                inter_model,
                num_layers=num_layers,
                norm=norm,
                K=int(K/(2**(self.num_blocks*0.5))),
                num_spks=1,
                skip_around_intra=skip_around_intra,
                linear_layer_after_inter_intra=linear_layer_after_inter_intra,
                use_global_pos_enc=use_global_pos_enc,
    ...
    def forward(self, x):
        ...
        res = L % (2 ** self.num_blocks)
        if res > 0:
            pad = torch.zeros(B, N, 2 ** self.num_blocks - res).to(x.device)
            # [B, N, L']
        ...
        # Bottlensck Sepformer Block
        # [B, (2^n)*N, L'//(2^n)]
        x = self.bottleneck(x)
        
        # Upsample Sepformer Blocks
        for i in range(self.num_blocks):
            x = self.upsample[i](x, downsamples[self.num_blocks - 1 - i])
            x = self.upblocks[self.num_blocks - 1 - i](x)          

        if res > 0:
            x = x[:, :, :, :L]

        assert x.shape == torch.Size([self.num_spks, B, N, L]), (x.shape, [self.num_spks, B, N, L])
