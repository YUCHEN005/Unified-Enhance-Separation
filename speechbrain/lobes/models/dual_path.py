...
class Multi_Scale_Dual_Path_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    def __init__(
        ...
        super(Multi_Scale_Dual_Path_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
        ...
        self.dual_mdl = nn.ModuleList([nn.ModuleList([]) for _ in range(len(K))])
        for i in range(len(K)):
            for j in range(num_layers):
                self.dual_mdl[i].append(
                    copy.deepcopy(
                        Dual_Computation_Block(
                            intra_model,
                            inter_model,
        ...
        self.conv2d = nn.ModuleList([nn.Conv2d(out_channels, out_channels * num_spks, kernel_size=1) for _ in range(len(K))])
        self.end_conv1x1 = nn.Conv1d(out_channels, in_channels, 1, bias=False)
        self.prelu = nn.ModuleList([nn.PReLU() for _ in range(len(K))])
        self.proj = nn.Sequential(
            nn.Conv1d(len(K) * out_channels, out_channels, 1), nn.PReLU()
        )
        self.activation = nn.ReLU()
        # gated output layer
        self.output = nn.Sequential(
    ...
    def forward(self, x):
        ...
        for i in range(len(self.K)):
            # [B, N, K, S]
            xi, gapi = self._Segmentation(x, self.K[i])

            # [B, N, K, S]
            for j in range(self.num_layers):
                xi = self.dual_mdl[i][j](xi)
            xi = self.prelu[i](xi)
            ...
            # [B*spks, N, K, S]
            xi = xi.view(B * self.num_spks, -1, K, S)

            # [B*spks, N, L]
            xi = self._over_add(xi, gapi)

            #print(f"x{i}: {xi.shape}")
            cache.append(xi)
        
        # [B*spks, 3*N, L]
        x = torch.cat(cache, dim=1).to(xi.device)
...
class Dilated_Dual_Path_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> dilated_Dual_Path_Model = Dilated_Dual_Path_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = dilated_Dual_Path_Model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    """
    ...
    def __init__(
        ...
        super(Dilated_Dual_Path_Model, self).__init__()
        self.K = K
        ...
        self.dual_mdl = nn.ModuleList([nn.ModuleList([]) for _ in range(num_dilated)])
        ...
        self.conv2d = nn.ModuleList([nn.Conv2d(out_channels, out_channels, kernel_size=1) for _ in range(num_dilated-1)])
        self.conv2d.append(nn.Conv2d(out_channels, out_channels * num_spks, kernel_size=1))

        self.end_conv1x1 = nn.Conv1d(out_channels, in_channels, 1, bias=False)
        self.prelu = nn.ModuleList([nn.PReLU() for _ in range(num_dilated)])
        self.activation = nn.ReLU()
        # gated output layer
        self.output = nn.Sequential(
            nn.Conv1d(out_channels, out_channels, 1), nn.Tanh()
    ...
    def forward(self, x):
        ...
        for i in range(self.num_dilated):
            ...
            # [B, N, K*d, S/d]
            x, gap = self._Segmentation(x, self.K * d)
            # [B, N, K, S]
            x = self._fold(x, d)

            # [B, N, K, S]
            for j in range(self.num_layers):
                x = self.dual_mdl[i][j](x)
            x = self.prelu[i](x)

            # [B, N, K, S] or [B, N*spks, K, S]
            x = self.conv2d[i](x)
            
            # [B, N, K, S] or [B*spks, N, K, S]
            if i == self.num_dilated-1:
                _, _, K, S = x.shape                
                x = x.view(B * self.num_spks, -1, K, S)

            # [B, N, K*d, S/d] or [B*spks, N, K*d, S/d]
            x = self._unfold(x, d)
    ...
    def _fold(self, input, d):
        """
        input: [B, N, K*d, S/d]
        output: [B, N, K, S]
        """
        B, N, K, S = input.shape
        input = input.view(B, N, -1, d, S).transpose(3, 4).contiguous().view(B, N, K//d, -1)

        return input

    def _unfold(self, input, d):
        """
        input: [B, N, K, S]
        output: [B, N, K*d, S/d]
        """
        B, N, K, S = input.shape
        input = input.view(B, N, K, -1, d).transpose(3, 4).contiguous().view(B, N, -1, S//d)
