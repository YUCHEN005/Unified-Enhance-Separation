...
from speechbrain.lobes.models.transformer.Conformer import ConvolutionModule, ConformerEncoder
from speechbrain.nnet.attention import RelPosEncXL
from speechbrain.nnet.activations import Swish
import speechbrain.nnet.RNN as SBRNN

from speechbrain.lobes.models import norms
...
class Conv1DBlock(nn.Module):
    ...
    def __init__(
        ...
        self.res_conv = nn.Conv1d(hid_chan, hid_chan, 1)
...
class TDConvNet(nn.Module):
    """Temporal Dilated Convolutional network used in ConvTasnet.
    Args:
    ...
    def __init__(
        ...
        bottleneck_conv = nn.Conv1d(in_chan, hid_chan, 1)
        ...
        mask_conv_inp = skip_chan if skip_chan else hid_chan
        mask_conv = nn.Conv1d(mask_conv_inp, mask_conv_inp, 1)
    ...
    def forward(self, mixture_w):
        ...
        Returns:
            :class:`torch.Tensor`: estimated mask of shape $(batch, nfilters, nframes)$
        """
        # [B, N, L]
        B, N, L = mixture_w.size()
...
class DenseBlock(nn.Module):
    ...
    Args:
        ...
        hid_chan (int, optional): Number of channels in the convolutional
            blocks.
        skip_chan (int, optional): Number of channels in the skip connections.
            If 0 or None, DenseBlock won't have any skip connections and the
            masks will be computed from the residual output.
            Corresponds to the ConvTasnet architecture in v1 or the paper.
        n_repeats (int, optional): Number of repeats. Defaults to 3.
        n_blocks (int, optional): Number of convolutional blocks in each
    ...
    def __init__(
        ...
        self.bottleneck = nn.Sequential(layer_norm, bottleneck_conv)
        # Succession of Conv1DBlock with exponentially increasing dilation.
        self.TCN = nn.ModuleList([nn.ModuleList() for r in range(n_repeats)])
        for r in range(n_repeats):
            for x in range(n_blocks):
                padding = (conv_kernel_size - 1) * 2 ** x // 2
                self.TCN[r].append(
        ...
        for r in range(self.n_repeats):
            ...
            for layer in self.TCN[r]:
                ...
                if self.skip_chan:
                    ...
                    output = tcn_out
                prev = torch.cat([output, prev], dim=1)
...
class SBConformerBlock(nn.Module):
    """A wrapper for the SpeechBrain implementation of the transformer encoder.
    ...
    def __init__(
        ...
        super(SBConformerBlock, self).__init__()
        self.use_positional_encoding = use_positional_encoding
...
class Dual_Computation_Conv1d_Block(nn.Module):
    """Computation block for dual-path processing.
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = Dual_Computation_Conv1d_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(Dual_Computation_Conv1d_Block, self).__init__()
        ...
        # Conv1d
        self.conv1 = nn.Conv1d(out_channels, out_channels, kernel, stride=1, padding=(kernel-1)//2)
        self.conv2 = nn.Conv1d(out_channels, out_channels, kernel, stride=1, padding=(kernel-1)//2)
    ...
    def forward(self, x):
        ...
        # [BS, K, N]
        intra = intra + self.conv1(intra.transpose(1,2).contiguous()).transpose(1,2).contiguous()
        ...
        # [BK, S, N]
        inter = inter + self.conv2(inter.transpose(1,2).contiguous()).transpose(1,2).contiguous()
...
class Dual_Computation_Conv2d_Block(nn.Module):
    """Computation block for dual-path processing.
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = Dual_Computation_Conv2d_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(Dual_Computation_Conv2d_Block, self).__init__()
        ...
        K1, S1 = kernel1
        K2, S2 = kernel2       
        self.conv1 = nn.Conv2d(out_channels, out_channels, (K1, S1), stride=(1, 1), padding=((K1-1)//2, (S1-1)//2))
        self.conv2 = nn.Conv2d(out_channels, out_channels, (K2, S2), stride=(1, 1), padding=((K2-1)//2, (S2-1)//2))
...
class Dual_Computation_Dense_Block(nn.Module):
    """Computation block for dual-path processing.
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = Dual_Computation_Dense_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(Dual_Computation_Dense_Block, self).__init__()
        ...
        # DConv
        self.intra_dconv = DenseBlock(
            in_chan=out_channels,
            hid_chan=out_channels,
            skip_chan=skip_channels,
            n_repeats=3,
            n_blocks=8,       
            conv_kernel_size=3,
            norm_type="gLN"
    ...
    def forward(self, x):
        ...
        intra = self.intra_dconv(intra)

        # [B, S, N, K]
        intra = intra.view(B, S, N, K)
        # [B, N, K, S]
        intra = intra.permute(0, 2, 3, 1).contiguous()
        if self.norm is not None:
            intra = self.intra_norm(intra)
...
class Dual_Computation_DConv_Block(nn.Module):
    """Computation block for dual-path processing.
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = Dual_Computation_DConv_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(Dual_Computation_DConv_Block, self).__init__()
...
class Dual_Computation_DConv1_Block(nn.Module):
    """Computation block for dual-path processing.
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = Dual_Computation_DConv1_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(Dual_Computation_DConv1_Block, self).__init__()
...
class Dual_Computation_DConv2_Block(nn.Module):
    """Computation block for dual-path processing.
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = Dual_Computation_DConv2_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(Dual_Computation_DConv2_Block, self).__init__()
...
class Dual_Path_Conv1d_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> dual_Path_Conv1d_Model = Dual_Path_Conv1d_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = dual_Path_Conv1d_Model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    """
    ...
    def __init__(
        ...
        super(Dual_Path_Conv1d_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
...
class Dual_Path_Conv2d_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> dual_Path_Conv2d_Model = Dual_Path_Conv2d_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = dual_Path_Conv2d_Model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    """
    ...
    def __init__(
        ...
        super(Dual_Path_Conv2d_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
...
class Dual_Path_Dense_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> dual_path_model = Dual_Path_Dense_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = dual_path_model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    ...
    def __init__(
        ...
        super(Dual_Path_Dense_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
...
class Dual_Path_DConv_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> dual_path_model = Dual_Path_DConv_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = dual_path_model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    ...
    def __init__(
        ...
        super(Dual_Path_DConv_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
...
class Dual_Path_DConv1_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> dual_path_model = Dual_Path_DConv1_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = dual_path_model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    ...
    def __init__(
        ...
        super(Dual_Path_DConv1_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
...
class Dual_Path_DConv2_Model(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> dual_path_model = Dual_Path_DConv2_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = dual_path_model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    ...
    def __init__(
        ...
        super(Dual_Path_DConv2_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
