...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.conv3(x)
        x3 = F.relu(x3)
        # print(f"encoder x1.shape: {x1.shape}")
        # print(f"encoder x2.shape: {x2.shape}")
        # print(f"encoder x3.shape: {x3.shape}")
...
class MultiScaleDecoder(nn.Module):
    def __init__(self, in_channels=256, out_channels=1, kernel_size=16, stride=8, bias=False):
    ...
    def forward(self, x):   # [[B, N, L1], [B, N, L2], [B, N, L3]]
        x1, x2, x3 = x
        ...
        x2 = self.decoder2(x2)
        x3 = self.decoder3(x3)
        # print(f"decoder x1.shape: {x1.shape}")
        # print(f"decoder x2.shape: {x2.shape}")
        # print(f"decoder x3.shape: {x3.shape}")
...
class Multi_Scale_Dual_Path_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(x)):
            ...
            xi = self.conv1d(xi)
            if self.use_global_pos_enc:
                xi = self.pos_enc(xi.transpose(1, -1)).transpose(1, -1) + xi * (
                    xi.size(1) ** 0.5
                )
      
            # [B, N, K, S]
            xi, gapi = self._Segmentation(xi, self.K)

            # [B, N, K, S]
            for j in range(self.num_layers):
                xi = self.dual_mdl[j](xi)
            xi = self.prelu(xi)
            ...
            # [B*spks, N, L]
            xi = self.output(xi) * self.output_gate(xi)

            # [B*spks, N, L]
            xi = self.end_conv1x1(xi)

            # [B, spks, N, L]
            _, N, L = xi.shape
            xi = xi.view(B, self.num_spks, N, L)
        ...
        # print(f"cache1.shape: {cache[0].shape}")
        # print(f"cache2.shape: {cache[1].shape}")
        # print(f"cache3.shape: {cache[2].shape}")
