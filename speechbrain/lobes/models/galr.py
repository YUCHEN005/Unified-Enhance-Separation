...
from speechbrain.lobes.models.transformer.Transformer_GALR import TransformerEncoder
from speechbrain.lobes.models.transformer.Transformer_GALR import PositionalEncoding
...
class GALR_Block(nn.Module):
    ...
    ---------
        ...
        >>> dual_comp_block = GALR_Block(intra_block, inter_block, 64)
    ...
    def __init__(
        ...
        linear_layer_after_intra=True,
        linear_layer_after_inter=False,
    ):
        super(GALR_Block, self).__init__()

        self.intra_mdl = intra_mdl
        self.inter_mdl = inter_mdl
        self.skip_around_intra = skip_around_intra
        self.linear_layer_after_intra = linear_layer_after_intra
        self.linear_layer_after_inter = linear_layer_after_inter

        # Norm
        self.norm = norm
        if norm is not None:
            self.intra_norm = select_norm(norm, out_channels, 4)
            self.mid_norm = nn.LayerNorm([out_channels])
            self.inter_norm = select_norm(norm, out_channels, 4)

        # low-dimension compression mapping
        self.map = nn.Linear(K, Q)
        self.map_inv = nn.Linear(Q, K)

        # Linear
        if linear_layer_after_intra:
    ...
    def forward(self, x):
        ...
        # [BS, K, N]
        if self.linear_layer_after_intra:
        ...
        # inter RNN
        # [B, N, S, K]
        inter = intra.permute(0, 1, 3, 2).contiguous()
        # [B, N, S, Q]
        inter = self.map(inter)
        Q = inter.shape[-1]

        # [B, Q, S, N]
        inter = inter.permute(0, 3, 2, 1)
        inter = self.mid_norm(inter)

        # [BQ, S, N]
        inter = inter.view(B * Q, S, N)
        
        # [BQ, S, N]
        inter = self.inter_mdl(inter)
        # [BQ, S, N]
        if self.linear_layer_after_inter:
            inter = self.inter_linear(inter)

        # [B, N, S, Q]
        inter = inter.view(B, Q, S, N).permute(0, 3, 2, 1).contiguous()
        # [B, N, S, K]
        inter = self.map_inv(inter)
        # [B, N, K, S]
...
class GAGA_Block(nn.Module):
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = GAGA_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(GAGA_Block, self).__init__()
        ...
        self.norm = norm
        if norm is not None:
            self.ahead_norm = nn.LayerNorm([out_channels])
            self.intra_norm = select_norm(norm, out_channels, 4)
            self.mid_norm = nn.LayerNorm([out_channels])
            self.inter_norm = select_norm(norm, out_channels, 4)

        # low-dimension compression mapping
        self.map1 = nn.Linear(K, Q)
        self.map_inv1 = nn.Linear(Q, K)
        self.map2 = nn.Linear(K, Q)
        self.map_inv2 = nn.Linear(Q, K)
    ...
    def forward(self, x):
        ...
        # intra RNN
        # [B, N, S, K]
        intra = x.permute(0, 1, 3, 2).contiguous()
        # [B, N, S, Q]
        intra = self.map1(intra)
        Q = intra.shape[-1]

        # [B, S, Q, N]
        intra = intra.permute(0, 2, 3, 1).contiguous()
        # [BS, Q, N]
        intra = self.ahead_norm(intra).view(B * S, Q, N)
        ...
        # [B, N, S, Q]
        intra = intra.view(B, S, Q, N).permute(0, 3, 1, 2).contiguous()
        # [B, N, S, K]
        intra = self.map_inv1(intra)
        # [B, N, K, S]
        intra = intra.permute(0, 1, 3, 2).contiguous()
        # if self.norm is not None:
        #     intra = self.intra_norm(intra)
        ...
        inter = intra.permute(0, 1, 3, 2).contiguous()
        # [B, N, S, Q]
        inter = self.map2(inter)
        Q = inter.shape[-1]

        # [BQ, S, N]
        inter = inter.permute(0, 3, 2, 1).contiguous()
        inter = self.mid_norm(inter).view(B * Q, S, N)
...
class LOGO_Block(nn.Module):
    ...
    ---------
        >>> intra_block = SBTransformerBlock(1, 64, 8)
        >>> inter_block = SBTransformerBlock(1, 64, 8)
        >>> dual_comp_block = LOGO_Block(intra_block, inter_block, 64)
        >>> x = torch.randn(10, 64, 100, 10)
        >>> x = dual_comp_block(x)
        >>> x.shape
        torch.Size([10, 64, 100, 10])
    ...
    def __init__(
        ...
        super(LOGO_Block, self).__init__()
...
class GALR_Model(nn.Module):
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> galr_model = GALR_Model(64, 64, intra_block, inter_block, num_spks=2)
    ...
    def __init__(
        ...
        self.galr_blocks = nn.ModuleList([])
        for i in range(num_layers):
            self.galr_blocks.append(
                copy.deepcopy(
                    GALR_Block(
                        intra_model,
                        inter_model,
                        ...
                        linear_layer_after_intra=linear_layer_after_intra,
                        linear_layer_after_inter=linear_layer_after_inter,
                    )
                )
            )
...
class GAGA_Model(nn.Module):
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> gaga_model = GAGA_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = gaga_model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    ...
    def __init__(
        ...
        super(GAGA_Model, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
...
class LOGO_Model(nn.Module):
    ...
    >>> intra_block = SBTransformerBlock(1, 64, 8)
    >>> inter_block = SBTransformerBlock(1, 64, 8)
    >>> galr_model = LOGO_Model(64, 64, intra_block, inter_block, num_spks=2)
    >>> x = torch.randn(10, 64, 2000)
    >>> x = galr_model(x)
    >>> x.shape
    torch.Size([2, 10, 64, 2000])
    ...
    def __init__(
        ...
        self.logo_blocks = nn.ModuleList([])
        for i in range(num_layers):
            self.logo_blocks.append(
                copy.deepcopy(
                    LOGO_Block(
                        intra_model,
                        inter_model,
...
#         # intra RNN
#         # [B, S, K, N]
#         intra = x.permute(0, 3, 2, 1).contiguous()
#         # [BS, K, N]
#         intra = self.ahead_norm(intra).view(B * S, K, N)
